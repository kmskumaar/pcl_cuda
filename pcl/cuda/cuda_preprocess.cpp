...
template <typename T>
void cuda::PreProcess<T>::normalEstimation(pcl::PointCloud<T> &inCloud, pcl::NormalCloud<T> &outNormal, const int neighbors) {
	...
	concurrency::parallel_for(0, static_cast<int>(inCloud.size()), [&flannIndices, &inCloud, &outNormal, neighbors, this](int idx) {
		...
		outNormal[idx] = this->computePointNormal(inCloud[idx], inCloud, neighborIndices);
	});
	//if (threadToUse != 1) {
...
template <typename T>
void cuda::PreProcess<T>::normalEstimation(pcl::PointCloud<T> &inCloud, pcl::NormalCloud<T> &outNormal, const float radius, const int max_nn) {
	outNormal.resize(inCloud.size());
	...
	concurrency::parallel_for(0, static_cast<int>(inCloud.size()), [&flannIndices, &inCloud, &outNormal, max_nn, this](int idx) {
		pcl::Indices neighborIndices;
		neighborIndices.indices.clear();
...
template void cuda::PreProcess<float>::normalEstimation(pcl::PointCloud<float> &inCloud, pcl::NormalCloud<float> &outNormal, const int neighbors);
template void cuda::PreProcess<float>::normalEstimation(pcl::PointCloud<float> &inCloud, pcl::NormalCloud<float> &outNormal, const float radius, const int max_nn);